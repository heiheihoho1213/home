---
title: '新版React官方文档解读（一）- Hooks 之 useCallback 和 useMemo'
slug: 'react-18-use-1'
date: '2023-07-01'
readTime: '10 min'
excerpt: '介绍 React 18 的 useCallback 和 useMemo ...'
...
- 只能在函数式组件顶部使用
- 在严格模式下，React会调用计算函数两次，以帮助您查找意外杂质。这只是**开发行为**，不影响生产。如果您的计算函数是纯的，这不会影响组件的逻辑。其中一个调用的结果将被忽略。
